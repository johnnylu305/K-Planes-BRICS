...
def save_config(config, res):
    root = os.path.join('plenoxels', 'configs_exp_c', 'final', 'Brics', res, config['objname'].split('-')[0])
...
if __name__=="__main__":
    ...
    objects = [f"battery_{res}", f"bunny_{res}", f"clock_{res}", f"music_box_{res}", f"horse_{res}", f"hour_glass_{res}", f"scissor_{res}", f"wolf_{res}", f"world_globe_{res}"]
    ...
    for obj in objects:
        ...
        log_dir = os.path.join(data_root, obj, "dynamic_data", "kplanes_exp_c", res)
        # make config for ist
